    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-white mb-2" style="font-family: 'Playfair Display', serif;">
                <i class="fas fa-hashtag text-amber-400"></i> Event Tags
            </h1>
            <p class="text-gray-400">Manage event tags and keywords</p>
        </div>
    ...
    <div class="p-6">
        <div class="mb-6 flex items-center justify-between">
            <h2 class="text-2xl font-bold text-white flex items-center">
                <i class="fas fa-list text-amber-400 mr-3"></i> Tags List
            </h2>
            <span class="text-sm text-gray-400">Total: <span class="text-amber-400 font-bold">{{ $tags->total() }}</span></span>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-slate-700/50 border-b-2 border-slate-600/50">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-amber-400 uppercase">Tag Name</th>
                        ...
                        <tr class="hover:bg-slate-700/20 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-3">
                                    <button onclick="openEditModal({{ $tag->id }}, '{{ addslashes($tag->name) }}', '{{ addslashes($tag->description ?? '') }}')" class="text-emerald-400 hover:text-emerald-300 transition-colors" title="Edit">
    ...
    <div class="card-luxury p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-4xl font-bold text-amber-400">{{ $tags->total() }}</p>
            </div>
...
<div id="importModal" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
    <div class="card-luxury rounded-2xl p-8 max-w-md w-full mx-4">
        <h3 class="text-2xl font-bold text-white mb-6">Import Tags from Excel/CSV</h3>
...
function openEditModal(id, name, description) {
    ...
    document.getElementById('editForm').action = `/admin/event-tags/${id}`;
